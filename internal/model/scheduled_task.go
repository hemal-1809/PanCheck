...
type ScheduledTask struct {
	...
	AutoDestroyAt   *time.Time  `gorm:"type:datetime" json:"auto_destroy_at"`                      // 自我销毁时间
	Status          string      `gorm:"type:varchar(20);not null;default:'stopped'" json:"status"` // active/stopped/expired
	LastRunAt       *time.Time  `gorm:"type:datetime" json:"last_run_at"`                          // 最后执行时间
	NextRunAt       *time.Time  `gorm:"type:datetime" json:"next_run_at"`                          // 下次执行时间
